...
var dispAdminListPage = function (res) {
  console.log("hit the admin list page");

  var listOutput = [];

  var queryStr = "SELECT * FROM administrators";

  function listOutputObj(_id, _name_hash, _name, _email_hash, _email, _password_hash) {
    this.id = _id;
    this.name_hash = _name_hash;
    this.name = _name;
  ...
  connection.query(queryStr, [], function (err, response) {
    ...
    for (var i = 0; i < response.length; i++) {
      //loop thru all of the responses
      var testStr = response[i].adminName_hash;
      ...
      var adminNameDecrypt = loginMod.decrypt(testStr);
      ...
      var adminEmail = "";
      testStr = response[i].email_hash;
      //encryption / decrypt can not be done on null strings
      if (testStr === undefined || testStr === null || testStr === "") {
        testStr = loginMod.encrypt(" ");
        adminEmail = loginMod.encrypt(testStr);
      };

      listOutput.push(new listOutputObj(
        response[i].id,
        response[i].adminName_hash,
        adminNameDecrypt,
    ...
    res.render('../app/views/admin/adminlist', {
      outputLines: listOutput
    });
  });
};
...
router.get('/admin_list', function (req, res) {
  console.log("hit the admin list post");
  dispAdminListPage(res);
});
...
router.post('/create', function (req, res) {
  ...
  if (loginMod.hackCheck(connection, transporterObj, userName, userEmail, userPassword, userPassword2)) {
    userName = "invalid";
    userEmail = "invalid";
  ...
  var sendObjBack = function (errCode, errMsg, errLine, errExp, _user_name, _user_email) {
    auditMod.writeAuditLog(connection, "create user", _user_name, _user_email, "code: " + errCode + " = " + errMsg, " ", " ");
    respondObj.errCode = errCode;
    respondObj.errLine = errLine;
    respondObj.errMsg = errMsg;
    respondObj.errExp = errExp;
...
//create a new administrator -- can only reach thru admin menu
router.post('/createadmin', function (req, res) {
  ...
  var sendObjBack = function (errCode, errMsg, errLine, errExp, _user_name, _user_email) {
    auditMod.writeAuditLog(connection, "create admin", _user_name, _user_email, "code: " + errCode + " = " + errMsg, " ", " ");
    respondObj.errCode = errCode;
    respondObj.errLine = errLine;
    respondObj.errMsg = errMsg;
    respondObj.errExp = errExp;
  ...
  connection.query(query, [emailEnc], function (err, response) {
    ...
    if (response != undefined && response != null && response.length > 0) {
      ...
      bcrypt.genSalt(10, function (err, salt) {
        bcrypt.hash(userPassword, salt, function (err, hash) {
          var query = "INSERT INTO administrators (adminName_hash, email_hash, password_hash  ) VALUES (?, ?, ? )";
          var emailEnc = loginMod.encrypt(userEmail);
          var nameEnc = loginMod.encrypt(userName);
          connection.query(query, [nameEnc, emailEnc, hash], function (err, response) {
            //need to add error 
            console.log(err);
            //now need to requery to make sure that the user just entered to get the user id
            var query = "SELECT * FROM administrators WHERE email_hash = ?";
